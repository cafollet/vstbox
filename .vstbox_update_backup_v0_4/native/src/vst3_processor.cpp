...
private:
    ...
    void prepare_events(std::uint64_t callback_index) noexcept {
        events_.clear();
        if (!midi_.enabled || component_->getBusCount(kEvent, kInput) <= 0 || midi_.cycle_callbacks == 0) return;
        ...
        if (midi_.gate_callbacks < midi_.cycle_callbacks && phase == midi_.gate_callbacks) {
            Event event{};
            event.busIndex = 0;
            event.sampleOffset = 0;
            event.ppqPosition = 0.0;
            ...
            event.noteOff.pitch = midi_.note;
